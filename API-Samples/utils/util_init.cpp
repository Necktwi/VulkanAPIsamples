...
void init_uniform_buffer(struct sample_info &info) {
    ...
    info.View = glm::lookAt(glm::vec3(0, 0, 5),  // Camera is at (-5,3,-10), in World Space
...
void init_vertex_buffer(struct sample_info &info, const void *vertexData, uint32_t dataSize, uint32_t dataStride,
                        bool use_texture, bool use_color) {
    VkResult U_ASSERT_ONLY res;
    bool U_ASSERT_ONLY pass;
    ...
    info.vi_attribs[1].format = use_texture ? VK_FORMAT_R32G32_SFLOAT : VK_FORMAT_R32G32B32A32_SFLOAT;
     info.vi_attribs[1].offset = use_color?16:0;
}

void init_index_buffer(struct sample_info &info, const void *indexData, uint32_t dataSize, uint32_t dataStride,
                        bool use_texture) {
    VkResult U_ASSERT_ONLY res;
    bool U_ASSERT_ONLY pass;
    ...
    buf_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    buf_info.flags = 0;
    res = vkCreateBuffer(info.device,  &buf_info, NULL, &info.index_buffer.buf);
    assert(res == VK_SUCCESS);

    VkMemoryRequirements mem_reqs;
    vkGetBufferMemoryRequirements(info.device, info.index_buffer.buf, &mem_reqs);
    ...
    res = vkAllocateMemory(info.device, &alloc_info, NULL, &(info.index_buffer.mem));
    assert(res == VK_SUCCESS);
    info.index_buffer.buffer_info.range = mem_reqs.size;
    info.index_buffer.buffer_info.offset = 0;

    uint8_t *pData;
    res = vkMapMemory(info.device, info.index_buffer.mem, 0, mem_reqs.size, 0, (void **)&pData);
    assert(res == VK_SUCCESS);

    memcpy(pData, indexData, dataSize);

    vkUnmapMemory(info.device, info.index_buffer.mem);

    res = vkBindBufferMemory(info.device, info.index_buffer.buf, info.index_buffer.mem, 0);
    assert(res == VK_SUCCESS);
...
void destroy_index_buffer(struct sample_info &info) {
    vkDestroyBuffer(info.device, info.index_buffer.buf, NULL);
    vkFreeMemory(info.device, info.index_buffer.mem, NULL);
}
